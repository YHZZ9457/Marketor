...
桌面应用包含关键指标卡、长期均线图、加减仓信号解释、指数切换和历史收益表。点击“指数排名”可横向比较；双击排名中的指数可查看 `MA250 乖离≤-10%` 的独立历史事件。右上角可即时切换“墨绿夜色、深海蓝、石墨紫、象牙日光”四套主题，选择会自动保存。FastAPI 网页与 HTTP API 仍作为可选开发接口保留。
